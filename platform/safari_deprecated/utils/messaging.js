...
if (popoverScript || backgroundScript) {
  ...
  eventEmitter.addEventListener('goldtime.message', function(event) {
...
export function send(message_type, payload) {
  ...
  if (contentScript) {
    ...
    const event = new CustomEvent('goldtime.message', { detail: Object.assign({ type: message_type }, payload) });
